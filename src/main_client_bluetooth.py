...
def main( args ):
   log = Logger("client_bluetooth.log")
   ...
   repeater = BlueRepeater( config, log )
   thread_repeater = Thread( target = repeater.mainloop, () )
   thread_command  = Thread( target = repeater.mainloop, () )
...
class BlueClient:
  def __init__(self, config, log ):
     ...
     self.port_control = int( config["client_blue_port"] )
     self.port_data    = int( config["client_blue_port"] ) + 1
